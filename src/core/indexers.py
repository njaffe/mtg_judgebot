...
class DatabaseIndexer:
    ...
    def __init__(
        ...
        Args:
            ...
            raw_docs_path: Path to raw documents (relative to data_path)
            embedding_model: Embedding model name (defaults from env or sensible default)
        """
        ...
        if not all([self.openai_api_key, self.faiss_index_path, self.data_path, self.raw_docs_path]):
            raise ValueError("All database configuration parameters are required")

        self.client = OpenAI(api_key=self.openai_api_key)

    def _load_single_large_text_file(self, file_path: str) -> List[Dict[str, Any]]:
        """Load a single large text file."""
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
        document = {"content": content, "metadata": {"filename": os.path.basename(file_path)}}
    ...
    def _embed_texts(self, texts: List[str]):
        """Embed a list of texts using OpenAI's Embedding API (direct SDK call)."""
        embeddings = []
        batch_size = 1000
